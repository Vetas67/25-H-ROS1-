...
def detect(detection_num):
    ...
    for ref_img in reference_images:
        category_count[ref_img['category']] = {'name':ref_img['category'], 'count':0, 'PX':0, 'PY':0}
    ...
    for count in range(1,detection_num):
        ...
        for ref in reference_images:
            ...
            # 更新计数数组数据
            category_count[ref['category']]['count']+=1
            category_count[ref['category']]['PX'] = (category_count[ref['category']]['PX']+cx)/category_count[ref['category']]['count']
            category_count[ref['category']]['PY'] = (category_count[ref['category']]['PY']+cy)/category_count[ref['category']]['count']
            
            #print(f"detection succeed : Category: {ref['category']}, imagine: {ref['name']}, position: ({cx}, {cy})")           
            #if SHOW_WINDOW:
            #    cv2.polylines(frame, [np.int32(projected)], True, (0, 255, 0), 2)
            #    cv2.putText(frame, ref['category'], (cx, cy), cv2.FONT_HERSHEY_SIMPLEX,
            #                0.8, (0, 255, 0), 2, cv2.LINE_AA)
            
    for res in category_count.values():
        if(res['count'] >= 0.02*detection_num):
            print(f"{res['name']} {res['PX']} {res['PY']}")
