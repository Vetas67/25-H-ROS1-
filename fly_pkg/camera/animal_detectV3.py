            ...
            if len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if(abs((x1-x2)*(y1 - y2)) > 6000):
                        cls = 4
                    if prob >= CONFIDENCE:
                        rospy.loginfo(f"category: {CLASSES[cls]}, Position: ({(x1 + x2)/2}, {(y1 + y2)/2}), Probability: {prob:.2f}")
                        # 更新计数数组数据
                        attend_flag = 1           
                        for it_count in range(len(category_count)):
                            item = category_count[it_count]
                            if((item['category'] == CLASSES[cls]) and ((item['PX'] - (x1+x2)/2)*(item['PX'] - (x1+x2)/2) + (item['PY'] - (y1+y2)/2)*(item['PY'] - (y1+y2)/2) < 20000):
...
def sub_callback(cmd_msg):
    ...
    publisher(res['category'], int(res['PX']), int(res['PY']), detect_count)
