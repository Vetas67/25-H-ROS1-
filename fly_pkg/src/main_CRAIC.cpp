...
ros::Subscriber throw_sub;
std_msgs::Int8 photo_control;
std_msgs::Int8 throw_control;
...
fly_pkg::robot_data object;
void cb_object(const fly_pkg::robot_data::ConstPtr &msg){
    object = *msg;
}
...
int main (int argc,char** argv){
    ...
    ros::Publisher pub_classify = nh.advertise<std_msgs::Int8>("classify_image",10);

    ros::Subscriber sub_object = nh.subscribe<fly_pkg::robot_data>("c_object",10,cb_object); 
    ...
    /*-------------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.6 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0 , 1.8, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/  
    ...
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 1.8, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    //开始识别
    classify.data = 1;
    pub_classify.publish(classify);
    classify.data = 0;
    pub_classify.publish(classify);

    //识别完毕 判断离开
    while(ros::ok()){
        if (object.is_leave == 1){
        //判断投掷
            if (object.is_drop == 1){
                fly_control_with_time(aim_pos,pos_client, 1.6 ,  1.8, 0.45 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                                3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
            ...
            fly_control_with_time(aim_pos,pos_client, 1.6 ,  3.6, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                            3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
            break;
        }
        ros::spinOnce();
    ...
    while(ros::ok()){
        ...
        //判断投掷
            if (object.is_drop == 1){
                fly_control_with_time(aim_pos,pos_client, 1.6 ,  3.6, 0.45 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                                3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
                system("echo 1 > /sys/class/gpio/PN.01/value");
                system("echo 1 > /sys/class/gpio/PCC.04/value");
                fly_control_with_time(aim_pos,pos_client, 1.6 ,  3.6, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
            ...
            fly_control_with_time(aim_pos,pos_client, 1.6 ,  2.7, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                            3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
            break;
        }
        ros::spinOnce();
    ...
    while(ros::ok()){
        ...
        //判断投掷
            ...
            fly_control_with_time(aim_pos,pos_client, -1.6 ,  6.0, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                            3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
            break;
        }
        ros::spinOnce();
    ...
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, -1 , 6.0 , 1.6 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);           //角度精度   位置精度  
    /*-------------------------------------------------------------------------------------------------*/
    //特殊靶投放 
    fly_control_with_time(aim_pos,pos_client, -1 , 6.0 , 0.45 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);           //角度精度   位置精度
    system("echo 1 > /sys/class/gpio/PN.01/value");
    system("echo 1 > /sys/class/gpio/PCC.04/value"); 
    /*-------------------------------------------------------------------------------------------------*/
    ...
    /*-------------------------------------------------------------------------------------------------*/
    //穿越圆环
    fly_control_with_time(aim_pos,pos_client, 2.3 , 6.0 , 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                         3*PI/180 , 0.07f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.3 , 0 , 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                         3*PI/180 , 0.07f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, down , 0.0, 1.6 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                         3*PI/180 , 0.07f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, down , 0.0, 0.0 , 0*PI/180 , Land,0.75f,100, //x y z yaw model time hz
                         3*PI/180 , 0.07f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度  
