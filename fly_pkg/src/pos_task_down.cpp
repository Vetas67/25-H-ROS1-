...
#include <tf/tf.h>
#include <math.h>
#include <fly_pkg/cv_data.h>
#include <boost/thread.hpp>
#include <cstdlib>

extern ros::Publisher led_pos_pub;
...
#define EEROR 3  //位置失灵
#define CV_task 4 //进入视觉任务
#define Special_Misson 5 //特殊飞行任务
int control_falg = NO; //控制指令

ros::Time last_times;
fly_pkg::pos_msg  aim_pos;
int cv_task_finish = 0; 
int special_task = 0;
std_msgs::Int8 led_pos;


bool special_task1_began = true;
bool special_task1_finish = false;
bool special_task2_began = false;
bool special_task2_finish = false;
...
//cv视觉回调函数,延时多次判断
fly_pkg::cv_data cv_data;
void cv_control_cb(const fly_pkg::cv_data::ConstPtr& msg) {
    cv_data = *msg;
}

//沙包任务结束后返回原高度
// bool cv_to_control_up() {
//     ROS_INFO("进入视觉回到原高度任务");
//     special_task2_began = false;
//     ros::Time last_times = ros::Time::now();
//     aim_pos.x = local_pos.pose.position.x;
//     aim_pos.y = local_pos.pose.position.y;
//     aim_pos.z = 1.8;   //降落高度
//     aim_pos.control_flag = ON;
//     ROS_INFO("准备回到原高度");
//     while(ros::ok()) {
...
//         {
//             if(ros::Time::now() -last_times> ros::Duration(0.75)){
//                 ROS_INFO("已回到原高度");
//                 special_task2_finish = false;
//                 return true;
//             }
//         else{
...
//视觉到达后控制降落
// bool cv_to_control_down() {
//     ROS_INFO("成功进入视觉降落任务");
//     special_task1_began = false;
//     ros::Time last_times = ros::Time::now();
//     aim_pos.x = local_pos.pose.position.x;
//     aim_pos.y = local_pos.pose.position.y;
//     aim_pos.z = 0.8;   //降落高度
...
//             if(ros::Time::now() -last_times> ros::Duration(0.75)){
//                 ROS_INFO("降落任务结束");
//                 special_task1_finish = true;
//                 special_task2_began = true;
//                 return true;
//             }
//         }
//         else{
...
bool pos_control_server(fly_pkg::pos_srv::Request& req,fly_pkg::pos_srv::Response& resp) { 
    ...
    //巡检任务与视觉任务区分
    ROS_INFO("视觉控制指令: %d,视觉任务完成指令:%d",cv_data.cv_control_flag,cv_task_finish);
    if((cv_data.cv_control_flag == 0 || cv_task_finish == 1) && special_task == 0){
        //巡检控制指令传递
        // if(cv_data.cv_control_flag==0){
        //     cv_task_finish = 0; //复位
        // }
        aim_pos.x = req.x;
        aim_pos.y = req.y;
    ...
    else if(cv_data.cv_control_flag == 1 && cv_task_finish == 0 && special_task == 0){
        //视觉控制指令传递
        aim_pos.control_flag = CV_task;
        system("echo 1 > /sys/class/gpio/PCC.04/value");  //GPIO1 高电平 
        if(fabs(320-cv_data.cv_x)<aim_pos.cv_pos_accuracy && fabs(240-cv_data.cv_y)<aim_pos.cv_pos_accuracy
        && fabs(local_pos.pose.position.z-aim_pos.z)<aim_pos.pos_accuracy)
        {   //角度yaw判断先忽略
            //到达视觉目标点后持续时间
            if(ros::Time::now() -last_times > ros::Duration(aim_pos.cv_times)){
                    cv_task_finish = 1; //直接永久结束cv视觉任务   后面在优化
                    special_task = 1;   //准备执行特殊飞行任务
            }
        }
        else{
            last_times = ros::Time::now();
            cv_task_finish = 0; //视觉任务未完成
        }
    }
    else if(special_task == 1 && special_task1_began == true && special_task1_finish == false) {
        if(special_task1_began){
            aim_pos.x = local_pos.pose.position.x;
            aim_pos.y = local_pos.pose.position.y;
            aim_pos.z = 0.95;   //降落高度
            aim_pos.control_flag = Special_Misson;
            if(fabs(local_pos.pose.position.x-aim_pos.x)<aim_pos.pos_accuracy &&
            fabs(local_pos.pose.position.y-aim_pos.y)<aim_pos.pos_accuracy &&
            fabs(local_pos.pose.position.z-aim_pos.z)<aim_pos.pos_accuracy)
            {
                if(ros::Time::now() -last_times> ros::Duration(2.0)){
                    ROS_INFO("已降落至1.0m");
                    led_pos.data = 1;
                    system("echo 1 > /sys/class/gpio/PN.01/value");  //GPIO2 高电平 
                    special_task1_finish = true;
                    special_task2_began = true;
                }
            else{
    ...
    else if(special_task == 1 && special_task2_began == true && special_task2_finish == false){
        if(special_task2_began){
            aim_pos.x = local_pos.pose.position.x;
            aim_pos.y = local_pos.pose.position.y;
            aim_pos.z = 1.8;   //上升高度
            aim_pos.control_flag = Special_Misson;
            ...
            fabs(local_pos.pose.position.z-aim_pos.z)<aim_pos.pos_accuracy)
            {
                if(ros::Time::now() -last_times> ros::Duration(0.75)){
                    ...
                    special_task2_began = true;
                    special_task = 0;
                    system("echo 0 > /sys/class/gpio/PN.01/value");   //GPIO2 低电平
                    system("echo 0 > /sys/class/gpio/PCC.04/value");  //GPIO1 低电平
                }
            else{
                last_times = ros::Time::now();
                }
...
int main (int argc,char** argv)
{
    ...
    setlocale(LC_ALL,"");
    ros::init(argc,argv,"pos_task_down");   //初始化ROS节点
    ...
    ros::Subscriber cv_control = nh.subscribe<fly_pkg::cv_data>("cv_task",10,cv_control_cb);
    ...
    ros::Publisher led_pos_pub = nh.advertise<std_msgs::Int8>("led_pos", 10);
    led_pos.data = 0;
    //位置
    ...
    while (ros::ok())
    {
        ...
        switch(control_falg){
            ...
            case CV_task:        
                ...
                //cv_PID输出速度值
                //vel.linear.x = positional_PID(&cv_x_pid_obj, &cv_x_pid_param);
                //vel.linear.y = positional_PID(&cv_y_pid_obj, &cv_y_pid_param);
                //vel.linear.z = positional_PID(&z_pid_obj, &z_pid_param);
                
                vel.linear.x = (cv_data.cv_x - 320) * 0.0015;
                vel.linear.y = (cv_data.cv_y - 240) * 0.0013;
                vel.linear.z = positional_PID(&z_pid_obj, &z_pid_param);
                ...
                ROS_INFO("aim_x=%f  loc_x= 320  aim_y=%f loc_y=240 aim_z=%f loc_z=%f",cv_data.cv_x, cv_data.cv_y,aim_pos.z, local_pos.pose.position.z);
