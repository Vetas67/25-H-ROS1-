...
// void cv_control(const fly_pkg::cv_data::ConstPtr& msg) {
//     cv_data = *msg;
// }
...
//外部声明
//extern bool cv_to_control_down();
...
int main (int argc,char** argv){
    ...
    // ros::Subscriber cv_control = nh.subscribe<fly_pkg::cv_data>("xxx",10,cv_control);       
    ...
    //采用服务通信的方式 
    //cv视觉判断: 如果标志位成立，则执行特殊飞行函数，若不成立，则继续飞行!!!
    //后期着重于交互这部分

    //回路
    //boost::thread cv_to_control_down();

    //当前角度为0 左边为正 右边为负 180 0 -180
    //正方形测试
    /*-------------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.8 , 0*PI/180 , ON, 0.75f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-----------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 3.2 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.8 , 3.2 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 0.8 , 0 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 0 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 3.2 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.4 , 3.2 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.4 , 0 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 3.2 , 0 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 3.2 , 3.2 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 4.0 , 3.2 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 4.0 , 0 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.8 , 0*PI/180 , ON, 0.25f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 30.0f,0.5f);          //巡检角度精度   巡检位置精度    图像角度精度   图像位置精度  到达图像后的持续时间
    /*-------------------------------------------------------------------------------------------------*/ 
